...
export function useAPY(): number {
  ...
  const startPrice = 990068149686353000n

  const { data: novETHPrice } = useReadContract({
    abi: lrtOracleAbi,
    address: contracts.lrtOracle,
    functionName: 'novETHPrice',
  }) as { data: bigint }
  ...
  const apy = calculateAPY(from, to, startPrice, novETHPrice)
  return apy * 100
}
